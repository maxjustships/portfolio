export const PROJECTS = [
    {
        title: "AI Studio Parser",
        url: "https://aistudio-parser.pages.dev/",
        description: "A utility to combine, reorder, and deduplicate conversation exports from [aistudio.google.com](https://aistudio.google.com/). Simplifies workflow by allowing drag-and-drop of JSON or TXT files.",
        tags: ["AI", "Tools", "platform:Web"],
        color: "orange",
        image: "assets/aistudioparser.webp"
    },
    {
        title: "Playlists Telegram Bot",
        url: "https://t.me/playlists_manager_bot",
        description: "Your music, where you already are. A minimalist Telegram bot that integrates your tracks directly into the messenger for a seamless, frictionless listening experience wherever you are.",
        tags: ["platform:Telegram", "Media", "UX"],
        color: "blue", 
        image: "assets/teleplaylist.webp"
    },
    {
        title: "Noise & Beats",
        url: "https://noiseandbeats.com/",
        description: "Minimalist audio engine for deep work. No distractions, just brown noise and rhythmic beats to keep you in the zone. Pure focus, zero friction.",
        tags: ["Media", "UX", "Tools", "platform:Web"],
        color: "orange",
        image: "assets/noiseandbeats.webp"
    },
    {
        title: "KBFlow",
        url: "https://kbflow.dev/",
        description: "A love letter to keyboard-driven mastery. Interactive challenges designed to break your mouse dependency and unlock the true speed of the keys.",
        tags: ["Education", "Tools", "platform:Web"],
        color: "blue", 
        image: "assets/kbflow.webp"
    }
];

export const CONTACT = {
    email: "reishmax@gmail.com"
};
